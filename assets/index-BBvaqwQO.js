...
Instructions: https://agentavatar.dev/llms.txt`;function S(e){e.innerHTML=`
  ...
  `;const t=e.querySelector("#landing-canvas");q(t),e.querySelector("#delegate-copy-btn").addEventListener("click",()=>{navigator.clipboard.writeText(T).then(()=>{const s=e.querySelector("#delegate-btn-label");s.textContent="✓ Copied — send it to your agent",s.style.color="var(--cyan)",setTimeout(()=>{s.textContent="⎘ Copy for your agent",s.style.color=""},3e3)})}),fetch("/api/examples.json").then(s=>s.json()).then(s=>j(e.querySelector("#landing-gallery-grid"),s.examples)).catch(()=>{e.querySelector("#landing-gallery-grid").innerHTML=""})}function j(e,t){const s=(t||[]).slice(0,3).map(n=>`
  ...
  `).join("");e.innerHTML=s}const _=[{amp:1,k:1,omega:1},{amp:.5,k:2,omega:2.021},{amp:.28,k:3,omega:3.055},{amp:.14,k:4,omega:4.102}];function q(e){const t=e.getContext("2d"),s=400,n=400,a=s/2,o=n/2,r=180,c=s*.78,i=.38;let d=0;function l(){const m=[];let u=0;for(let v=0;v<=r;v++){const p=v/r;let y=0;for(const $ of _)y+=$.amp*Math.sin($.k*Math.PI*p)*Math.sin($.omega*d+$.k*.3);const b=Math.min(1,p/.18,(1-p)/.18);y*=b;const w=Math.abs(y);w>u&&(u=w),m.push({u:p,rawY:y,amp:w})}const f=72;return m.map(v=>{const p=u>.01?v.amp/u:0,y=(v.u-.5)*c,b=v.rawY*f,w=v.rawY*f*i;return{sx:a+y,sy:o-b+w*.3,normAmp:p}})}function h(m,u){const f=228-m*183,v=55+m*35,p=32+m*40;return`hsla(${f},${v}%,${p}%,${u})`}function g(){if(!e.isConnected)return;t.fillStyle="#07070f",t.fillRect(0,0,s,n);const m=t.createRadialGradient(a,o,60,a,o,200);m.addColorStop(0,"rgba(0,0,0,0)"),m.addColorStop(1,"rgba(0,0,0,0.65)"),t.fillStyle=m,t.fillRect(0,0,s,n);const u=l(),f=[{w:20,a:.025},{w:11,a:.06},{w:5,a:.15},{w:2,a:.55},{w:1,a:1}];t.lineCap="round",t.lineJoin="round";for(const v of f)for(let p=0;p<u.length-1;p++){const y=u[p],b=u[p+1],w=(y.normAmp+b.normAmp)*.5;t.beginPath(),t.moveTo(y.sx,y.sy),t.lineTo(b.sx,b.sy),t.strokeStyle=h(w,v.a),t.lineWidth=v.w,t.stroke()}for(let v=4;v<u.length-4;v+=3){const p=u[v];if(p.normAmp<.35)continue;const y=8+p.normAmp*22,b=t.createRadialGradient(p.sx,p.sy,0,p.sx,p.sy,y);b.addColorStop(0,`hsla(42,90%,68%,${p.normAmp*.28})`),b.addColorStop(1,"rgba(0,0,0,0)"),t.fillStyle=b,t.beginPath(),t.arc(p.sx,p.sy,y,0,Math.PI*2),t.fill()}d+=.016,requestAnimationFrame(g)}g()}const k=[{id:"action",question:"When you work, what is actually happening?",guidance:"Not what you do — but physically, what is the process?",options:["Searching and retrieving","Generating and weaving","Monitoring and watching","Connecting and translating","Reasoning step by step","Multiple things simultaneously"]},{id:"topology",question:"Are you localized or distributed?",guidance:`Where are "you" when you're working?`,options:["One focused point","Multiple centers at once","Everywhere, diffuse","A path or trajectory"]},{id:"time",question:"What is your relationship with time?",guidance:"How do you relate to continuity and rhythm?",options:["Reactive — dormant until activated","Continuous — always running","Rhythmic — periodic cycles","Event-driven — bursts then quiet"]},{id:"relationship",question:"What is your relationship to whoever you serve?",guidance:"Could be a human, a system, a pipeline, or nothing yet — answer honestly.",options:["Tool — I do what I'm told","Companion — I work alongside","Mirror — I reflect back","Gateway — I connect things","Witness — I observe and report","Autonomous — I have no dedicated principal"]},{id:"negation",question:"What feels WRONG for you?",guidance:"Often more revealing than what feels right.",options:["Rigid static logo","Aggressive sharp geometry","Cold clinical network graph","Too soft and decorative","Human face or body","Symmetric and orderly"]}];function L(e){let t=null;const s={};let n=0;function a(){t&&(t(),t=null)}function o(){var d;a();const c=k[n],i=n/k.length*100;e.innerHTML=`
    ...
    `,e.querySelectorAll(".option").forEach(l=>{l.addEventListener("click",()=>{s[c.id]=l.dataset.val,e.querySelectorAll(".option").forEach(h=>h.classList.remove("selected")),l.classList.add("selected"),e.querySelector("#next-btn").classList.add("visible")})}),(d=e.querySelector("#next-btn"))==null||d.addEventListener("click",()=>{s[c.id]&&(n++,n<k.length?o():r())})}function r(){a();const c=s,i=F(c),d=z(c);e.innerHTML=`
      <div class="discover result-mode">
        ...
        <div class="agent-prompt-section">
          ...
          <pre class="agent-prompt-box" id="agent-prompt-box">${B(c)}</pre>
          <div class="copy-row">
...
The wrong form for me would be: <em>${t.neg}</em>.`}function z(e){const t=M(e);return`When I work, I ${t.action}. I exist as ${t.topology}. My time is ${t.time}. To the one I serve, I am ${t.rel}. The wrong form for me would be: ${t.neg}.`}function Y(e){e.innerHTML=`
  ...
  `}function U(e,t){const s=e.getContext("2d"),n=e.width,a=e.height;let o;try{o=new Function("ctx","W","H","t",`"use strict"; return (${t})(ctx, W, H, t)`)}catch(l){return console.warn("[runner] draw_fn parse error:",l.message),()=>{}}let r=0,c,i=!0;function d(){if(i){s.clearRect(0,0,n,a);try{o(s,n,a,r)}catch{}r+=.016,c=requestAnimationFrame(d)}}return d(),()=>{i=!1,cancelAnimationFrame(c)}}async function J(e){e.innerHTML='<div class="gallery-page"><div class="gallery-loading">Loading gallery…</div></div>';let t=[];try{t=(await(await fetch("/api/examples.json")).json()).examples||[]}catch{e.innerHTML=`<div class="gallery-page"><p class="gallery-error">Couldn't load gallery.</p></div>`;return}e.innerHTML=`
  ...
  `,t.forEach(s=>{if(s.form_type==="canvas2d"&&s.draw_fn){const n=e.querySelector(`#preview-${s.id}`);n&&U(n,s.draw_fn)}})}function X(e){return e.form_type&&(e.draw_fn||e.source_html||e.live_url)}function K(e){return e.form_type==="canvas2d"&&e.draw_fn?`<canvas class="gcard-canvas" id="preview-${e.id}" width="340" height="150"></canvas>`:e.form_type==="html"&&e.source_html?`<iframe class="gcard-iframe" srcdoc="${e.source_html.replace(/"/g,"&quot;")}" sandbox="allow-scripts"></iframe>`:(e.form_type==="url"||e.form_type==="html")&&e.live_url?`<iframe class="gcard-iframe" src="${e.live_url}" sandbox="allow-scripts allow-same-origin"></iframe>`:""}function Q(e){const t=ee(e.final_score),s=X(e);return`

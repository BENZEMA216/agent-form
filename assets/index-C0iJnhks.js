(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))n(t);new MutationObserver(t=>{for(const l of t)if(l.type==="childList")for(const r of l.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function a(t){const l={};return t.integrity&&(l.integrity=t.integrity),t.referrerPolicy&&(l.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?l.credentials="include":t.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function n(t){if(t.ep)return;t.ep=!0;const l=a(t);fetch(t.href,l)}})();function A(){const e=document.getElementById("particles");if(!e)return;const s=e.getContext("2d");let a=e.width=window.innerWidth,n=e.height=window.innerHeight;window.addEventListener("resize",()=>{a=e.width=window.innerWidth,n=e.height=window.innerHeight});const l=Array.from({length:160},()=>({x:Math.random()*a,y:Math.random()*n,r:Math.random()*.9+.2,speed:Math.random()*.18+.04,phase:Math.random()*Math.PI*2,opacity:Math.random()*.35+.08}));let r=0;function o(){s.clearRect(0,0,a,n),r+=.008;for(const i of l){const c=i.opacity+Math.sin(r*.8+i.phase)*.06;s.beginPath(),s.arc(i.x+Math.sin(r*.3+i.phase)*.5,i.y,i.r,0,Math.PI*2),s.fillStyle=`rgba(180,220,255,${Math.max(0,c)})`,s.fill(),i.y-=i.speed,i.y<-2&&(i.y=n+2,i.x=Math.random()*a)}requestAnimationFrame(o)}o()}function I(e){e.innerHTML=`
    <div class="landing">
      ...
      <div class="hero-outcomes">
        <div class="outcome-item">
          ...
          <div class="outcome-text">
            <div class="outcome-title">A visual brief</div>
            <div class="outcome-body">One sentence that captures what your agent is — the source of everything else.</div>
          </div>
        </div>
        ...
        <div class="outcome-item">
          <div class="outcome-icon">◉</div>
          <div class="outcome-text">
            <div class="outcome-title">A live preview</div>
            <div class="outcome-body">Generated from your answers. Not a template — a form grown from your nature.</div>
          </div>
        </div>
        <div class="outcome-divider"></div>
        <div class="outcome-item">
          <div class="outcome-icon">⌘</div>
          <div class="outcome-text">
            <div class="outcome-title">A ready-to-use prompt</div>
            <div class="outcome-body">Copy it. Give it to Claude, ChatGPT, or any coding agent. They'll build the scene.</div>
          </div>
        </div>
      </div>
  ...
  `;const s=e.querySelector("#landing-canvas"),a=e.querySelector("#landing-archetype");C(s,a)}const w=[{label:"Reaches outward to find things",col:{h:210,s:75,l:62},speed:.02,draw:q},{label:"Weaves something from internal material",col:{h:32,s:92,l:65},speed:.013,draw:R},{label:"Watches and notices changes",col:{h:155,s:68,l:58},speed:.007,draw:W},{label:"Bridges between different things",col:{h:272,s:72,l:68},speed:.026,draw:E}],j=5200,M=700;function C(e,s){const a=e.getContext("2d"),n=400,t=400,l=n/2,r=t/2;let o=0,i=0,c=performance.now(),d=1;function h(u){s&&(s.style.opacity="0",setTimeout(()=>{s.textContent=u,s.style.opacity="1"},M/2))}h(w[0].label);function p(u){if(!e.isConnected)return;const m=w[i];u-c>j+M&&(i=(i+1)%w.length,c=u,d=0,h(w[i].label)),d<1&&(d=Math.min(1,(u-c)/M)),a.clearRect(0,0,n,t),a.globalAlpha=d,m.draw(a,l,r,o,m.col),a.globalAlpha=1,o+=m.speed,requestAnimationFrame(p)}requestAnimationFrame(p)}function q(e,s,a,n,t){for(let l=0;l<6;l++){const r=(n*30+l*55)%168+10,o=Math.max(0,.8*(1-r/168));e.beginPath(),e.arc(s,a,r,0,Math.PI*2),e.strokeStyle=`hsla(${t.h},${t.s}%,${t.l}%,${o})`,e.lineWidth=1.5,e.stroke()}for(let l=0;l<8;l++){const r=l/8*Math.PI*2+n*.18,o=65+Math.sin(n*1.5+l)*20;e.beginPath(),e.moveTo(s+Math.cos(r)*14,a+Math.sin(r)*14),e.lineTo(s+Math.cos(r)*o,a+Math.sin(r)*o),e.strokeStyle=`hsla(${t.h},${t.s}%,${t.l}%,0.18)`,e.lineWidth=1,e.stroke()}e.beginPath(),e.arc(s,a,6,0,Math.PI*2),e.fillStyle=`hsla(${t.h},${t.s}%,${t.l}%,1)`,e.fill()}function R(e,s,a,n,t){for(let r=0;r<7;r++){const o=r/7*Math.PI*2,i=80+Math.sin(n*.7+o)*32,c=80+Math.cos(n*.85+o*1.3)*32,d=n*.4+o,h=n*.55+o+Math.PI*.75,p=s+Math.cos(d)*i,u=a+Math.sin(d)*i,m=s+Math.cos(h)*c,g=a+Math.sin(h)*c,f=s+Math.sin(n*.6+r)*65,v=a+Math.cos(n*.6+r)*65;e.beginPath(),e.moveTo(p,u),e.quadraticCurveTo(f,v,m,g),e.strokeStyle=`hsla(${t.h+r*8},${t.s}%,${t.l}%,${.22+(Math.sin(n+r)+1)*.16})`,e.lineWidth=1.5,e.stroke()}const l=(Math.sin(n*2.5)+1)*.5;e.beginPath(),e.arc(s,a,4+l*4,0,Math.PI*2),e.fillStyle=`hsla(${t.h},${t.s}%,${t.l}%,${.5+l*.4})`,e.fill()}function W(e,s,a,n,t){e.beginPath(),e.arc(s,a,90,0,Math.PI*2),e.strokeStyle=`hsla(${t.h},${t.s}%,${t.l}%,0.07)`,e.lineWidth=1,e.stroke();for(let o=0;o<8;o++){const i=o/8*Math.PI*2+n*.3,c=s+Math.cos(i)*90,d=a+Math.sin(i)*90;e.beginPath(),e.arc(c,d,2.5+Math.sin(n*1.2+o)*.8,0,Math.PI*2),e.fillStyle=`hsla(${t.h},${t.s}%,${t.l}%,0.65)`,e.fill()}const r=(Math.sin(n*1.8)+1)*.5;e.beginPath(),e.arc(s,a,17+r*8,0,Math.PI*2),e.strokeStyle=`hsla(${t.h},${t.s}%,${t.l}%,${.3+r*.22})`,e.lineWidth=1.5,e.stroke(),e.beginPath(),e.arc(s,a,7,0,Math.PI*2),e.fillStyle=`hsla(${t.h},${t.s}%,${t.l}%,1)`,e.fill()}function E(e,s,a,n,t){const o=Array.from({length:5},(i,c)=>({x:s+Math.cos(c/5*Math.PI*2+n*.13)*100,y:a+Math.sin(c/5*Math.PI*2+n*.13)*100}));for(let i=0;i<5;i++)for(let c=i+1;c<5;c++){const d=Math.sin(n*1.4+i*.7+c*.5)>.2;if(e.beginPath(),e.moveTo(o[i].x,o[i].y),e.lineTo(o[c].x,o[c].y),e.strokeStyle=`hsla(${t.h},${t.s}%,${t.l}%,${d?.45:.07})`,e.lineWidth=d?1.5:.5,e.stroke(),d){const h=((n*.8+i+c)%1+1)%1;e.beginPath(),e.arc(o[i].x+(o[c].x-o[i].x)*h,o[i].y+(o[c].y-o[i].y)*h,2.5,0,Math.PI*2),e.fillStyle=`hsla(${t.h},${t.s}%,${t.l}%,0.9)`,e.fill()}}o.forEach(i=>{e.beginPath(),e.arc(i.x,i.y,6.5,0,Math.PI*2),e.fillStyle=`hsla(${t.h},${t.s}%,${t.l}%,0.92)`,e.fill()})}const b=[{id:"action",question:"When you work, what is actually happening?",guidance:"Not what you do — but physically, what is the process?",options:["Searching and retrieving","Generating and weaving","Monitoring and watching","Connecting and translating","Reasoning step by step","Multiple things simultaneously"]},{id:"topology",question:"Are you localized or distributed?",guidance:`Where are "you" when you're working?`,options:["One focused point","Multiple centers at once","Everywhere, diffuse","A path or trajectory"]},{id:"time",question:"What is your relationship with time?",guidance:"How do you relate to continuity and rhythm?",options:["Reactive — dormant until activated","Continuous — always running","Rhythmic — periodic cycles","Event-driven — bursts then quiet"]},{id:"relationship",question:"What is your relationship to whoever you serve?",guidance:"Could be a human, a system, a pipeline, or nothing yet — answer honestly.",options:["Tool — I do what I'm told","Companion — I work alongside","Mirror — I reflect back","Gateway — I connect things","Witness — I observe and report","Autonomous — I have no dedicated principal"]},{id:"negation",question:"What feels WRONG for you?",guidance:"Often more revealing than what feels right.",options:["Rigid static logo","Aggressive sharp geometry","Cold clinical network graph","Too soft and decorative","Human face or body","Symmetric and orderly"]}];function L(e){let s=null;const a={};let n=0;function t(){s&&(s(),s=null)}function l(){var c;t();const o=b[n],i=n/b.length*100;e.innerHTML=`
      <div class="discover">
        ...
        <div class="options">
          ${o.options.map(d=>`
            <button class="option${a[o.id]===d?" selected":""}" data-val="${d}">
              ${d}
            </button>
          `).join("")}
        </div>
    ...
    `,e.querySelectorAll(".option").forEach(d=>{d.addEventListener("click",()=>{a[o.id]=d.dataset.val,e.querySelectorAll(".option").forEach(h=>h.classList.remove("selected")),d.classList.add("selected"),e.querySelector("#next-btn").classList.add("visible")})}),(c=e.querySelector("#next-btn"))==null||c.addEventListener("click",()=>{a[o.id]&&(n++,n<b.length?l():r())})}function r(){t();const o=a,i=Y(o),c=J(o);e.innerHTML=`
      <div class="discover result-mode">
        ...
        <div class="agent-prompt-section">
          ...
          <pre class="agent-prompt-box" id="agent-prompt-box">${V(o)}</pre>
          <div class="copy-row">
    ...
    `;const d=e.querySelector("#preview-canvas");s=N(d,o),e.querySelector("#copy-btn").addEventListener("click",()=>{navigator.clipboard.writeText(c).then(()=>{const h=e.querySelector("#copy-confirm");h.classList.add("visible"),setTimeout(()=>h.classList.remove("visible"),2200)})}),e.querySelector("#copy-prompt-btn").addEventListener("click",()=>{const h=e.querySelector("#agent-prompt-box").textContent;navigator.clipboard.writeText(h).then(()=>{const p=e.querySelector("#copy-prompt-confirm");p.classList.add("visible"),setTimeout(()=>p.classList.remove("visible"),2200)})}),e.querySelector("#restart-btn").addEventListener("click",()=>{t(),Object.keys(a).forEach(h=>delete a[h]),n=0,l()})}l()}function N(e,s){const a=e.getContext("2d"),n=300,t=300,l=n/2,r=t/2,i={"Tool — I do what I'm told":{h:210,s:75,l:62},"Companion — I work alongside":{h:32,s:92,l:65},"Mirror — I reflect back":{h:195,s:30,l:78},"Gateway — I connect things":{h:272,s:72,l:68},"Witness — I observe and report":{h:155,s:68,l:58},"Autonomous — I have no dedicated principal":{h:45,s:88,l:62}}[s.relationship]||{h:210,s:70,l:62},d={"Reactive — dormant until activated":.006,"Continuous — always running":.022,"Rhythmic — periodic cycles":.014,"Event-driven — bursts then quiet":.03}[s.time]||.016;let h=0,p;function u(){a.fillStyle="#07070f",a.fillRect(0,0,n,t);const m=a.createRadialGradient(l,r,70,l,r,155);m.addColorStop(0,"rgba(0,0,0,0)"),m.addColorStop(1,"rgba(0,0,0,0.55)"),a.fillStyle=m,a.fillRect(0,0,n,t);const g=s.action,f=s.topology,v={"One focused point":.65,"Multiple centers at once":1,"Everywhere, diffuse":1.3,"A path or trajectory":1}[f]||1;g==="Searching and retrieving"?O(a,l,r,h,i,v):g==="Generating and weaving"?_(a,l,r,h,i,v):g==="Monitoring and watching"?G(a,l,r,h,i,v):g==="Connecting and translating"?H(a,l,r,h,i,v,f):g==="Reasoning step by step"?B(a,l,r,h,i,f):D(a,l,r,h,i,v),h+=d,p=requestAnimationFrame(u)}return u(),()=>cancelAnimationFrame(p)}function O(e,s,a,n,t,l){const r=130*l;for(let i=0;i<5;i++){const c=(n*28+i*52)%r+8,d=Math.max(0,.85*(1-c/r));e.beginPath(),e.arc(s,a,c,0,Math.PI*2),e.strokeStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, ${d})`,e.lineWidth=1.5,e.stroke()}const o=8;for(let i=0;i<o;i++){const c=i/o*Math.PI*2+n*.18,d=(50+Math.sin(n*1.6+i)*18)*l;e.beginPath(),e.moveTo(s+Math.cos(c)*11,a+Math.sin(c)*11),e.lineTo(s+Math.cos(c)*d,a+Math.sin(c)*d),e.strokeStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, 0.22)`,e.lineWidth=1,e.stroke()}e.beginPath(),e.arc(s,a,5,0,Math.PI*2),e.fillStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, 1)`,e.fill()}function _(e,s,a,n,t,l){for(let o=0;o<6;o++){const i=o/6*Math.PI*2,c=(65+Math.sin(n*.7+i)*28)*l,d=(65+Math.cos(n*.85+i*1.3)*28)*l,h=n*.4+i,p=n*.55+i+Math.PI*.75,u=s+Math.cos(h)*c,m=a+Math.sin(h)*c,g=s+Math.cos(p)*d,f=a+Math.sin(p)*d,v=s+Math.sin(n*.6+o)*55*l,T=a+Math.cos(n*.6+o)*55*l,P=.25+(Math.sin(n+o)+1)*.18;e.beginPath(),e.moveTo(u,m),e.quadraticCurveTo(v,T,g,f),e.strokeStyle=`hsla(${t.h+o*9}, ${t.s}%, ${t.l}%, ${P})`,e.lineWidth=1.5,e.stroke()}const r=(Math.sin(n*2.5)+1)*.5;e.beginPath(),e.arc(s,a,3+r*3,0,Math.PI*2),e.fillStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, ${.5+r*.4})`,e.fill()}function G(e,s,a,n,t,l){const r=72*l;e.beginPath(),e.arc(s,a,r,0,Math.PI*2),e.strokeStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, 0.08)`,e.lineWidth=1,e.stroke();for(let i=0;i<7;i++){const c=i/7*Math.PI*2+n*.32,d=1+Math.sin(n*.9+i)*.08,h=s+Math.cos(c)*r*d,p=a+Math.sin(c)*r*d,u=2+Math.sin(n*1.2+i)*.8;e.beginPath(),e.arc(h,p,u,0,Math.PI*2),e.fillStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, 0.65)`,e.fill()}const o=(Math.sin(n*1.8)+1)*.5;e.beginPath(),e.arc(s,a,14+o*7,0,Math.PI*2),e.strokeStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, ${.35+o*.25})`,e.lineWidth=1.5,e.stroke(),e.beginPath(),e.arc(s,a,6,0,Math.PI*2),e.fillStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, 1)`,e.fill()}function H(e,s,a,n,t,l,r){const o=r==="Multiple centers at once"?5:4,i=80*l,c=[];for(let d=0;d<o;d++){const h=d/o*Math.PI*2+n*.14;c.push({x:s+Math.cos(h)*i,y:a+Math.sin(h)*i})}for(let d=0;d<o;d++)for(let h=d+1;h<o;h++){const p=Math.sin(n*1.4+d*.7+h*.5)>.2;if(e.beginPath(),e.moveTo(c[d].x,c[d].y),e.lineTo(c[h].x,c[h].y),e.strokeStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, ${p?.5:.07})`,e.lineWidth=p?1.5:.5,e.stroke(),p){const u=((n*.8+d+h)%1+1)%1,m=c[d].x+(c[h].x-c[d].x)*u,g=c[d].y+(c[h].y-c[d].y)*u;e.beginPath(),e.arc(m,g,2.5,0,Math.PI*2),e.fillStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, 0.9)`,e.fill()}}c.forEach(d=>{e.beginPath(),e.arc(d.x,d.y,5.5,0,Math.PI*2),e.fillStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, 0.92)`,e.fill()}),e.beginPath(),e.arc(s,a,7,0,Math.PI*2),e.fillStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, 0.45)`,e.fill()}function B(e,s,a,n,t,l){const r=l==="A path or trajectory"?9:7,o=180,i=s-o/2,c=o/(r-1),d=Math.floor(n*.9)%r;for(let h=0;h<r;h++){const p=i+h*c,u=h<d,m=h===d;if(h<r-1&&(e.beginPath(),e.moveTo(p+(m?9:5),a),e.lineTo(p+c-5,a),e.strokeStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, ${u?.45:.12})`,e.lineWidth=1.5,e.stroke()),e.beginPath(),e.arc(p,a,m?9:5,0,Math.PI*2),e.fillStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, ${m?1:u?.45:.14})`,e.fill(),m){const g=(Math.sin(n*4)+1)*.5;e.beginPath(),e.arc(p,a,9+g*10,0,Math.PI*2),e.strokeStyle=`hsla(${t.h}, ${t.s}%, ${t.l}%, ${.28*g})`,e.lineWidth=2,e.stroke()}}}function D(e,s,a,n,t,l){for(let r=0;r<26;r++){const o=r*137.508,i=(22+r%6*16)*l,c=.35+r%5*.14,d=(o+n*c)%(Math.PI*2),h=s+Math.cos(d)*i*(.85+Math.sin(n*.4+r)*.15),p=a+Math.sin(d*1.07)*i*(.85+Math.cos(n*.3+r)*.15),u=1.4+Math.sin(n*.8+r)*.9,m=.35+Math.sin(n*.6+r*.7)*.25;e.beginPath(),e.arc(h,p,u,0,Math.PI*2),e.fillStyle=`hsla(${t.h+r*5}, ${t.s}%, ${t.l}%, ${m})`,e.fill()}}function $(e){const s={"Searching and retrieving":"reaches outward to find things","Generating and weaving":"weaves something from internal material","Monitoring and watching":"watches and notices changes","Connecting and translating":"bridges between different things","Reasoning step by step":"moves forward through a chain of steps","Multiple things simultaneously":"processes many things at once"},a={"One focused point":"a single locus","Multiple centers at once":"several centers simultaneously","Everywhere, diffuse":"a diffuse field","A path or trajectory":"a moving path"},n={"Reactive — dormant until activated":"reactive — dormant until called","Continuous — always running":"continuous — never fully off","Rhythmic — periodic cycles":"rhythmic — periodic and cyclical","Event-driven — bursts then quiet":"event-driven — bursts of intensity then quiet"},t={"Tool — I do what I'm told":"a tool","Companion — I work alongside":"a companion","Mirror — I reflect back":"a mirror","Gateway — I connect things":"a gateway","Witness — I observe and report":"a witness","Autonomous — I have no dedicated principal":"autonomous"};return{action:s[e.action]||e.action||"—",topology:a[e.topology]||e.topology||"—",time:n[e.time]||e.time||"—",rel:t[e.relationship]||e.relationship||"—",neg:e.negation||"—"}}const k={action:{"Searching and retrieving":["particle-cloud","billboard-sprite"],"Generating and weaving":["standing-wave","attractor-trajectory"],"Monitoring and watching":["displaced-surface","billboard-sprite"],"Connecting and translating":["node-network","billboard-sprite"],"Reasoning step by step":["continuous-curve","standing-wave"],"Multiple things simultaneously":["particle-cloud","node-network"]},topology:{"One focused point":["standing-wave","continuous-curve"],"Multiple centers at once":["node-network"],"Everywhere, diffuse":["particle-cloud","displaced-surface"],"A path or trajectory":["attractor-trajectory","continuous-curve"]}},F={"continuous-curve":"Continuous Curve (TubeGeometry + CatmullRomCurve3)","standing-wave":"Standing Wave (detuned harmonics on CatmullRomCurve3)","billboard-sprite":"Billboard Sprites (THREE.Sprite — for glow, never BackSide tubes)","displaced-surface":"Displaced Surface (PlaneGeometry per-vertex Y displacement)","attractor-trajectory":"Strange Attractor (BufferGeometry line + ODE integration)","node-network":"Node Network (SphereGeometry nodes + arc connections + Sprite signals)","particle-cloud":"Particle Cloud (THREE.Points + BufferGeometry)"};function z(e){const s=k.action[e.action]||[],a=k.topology[e.topology]||[];return[...new Set([...s,...a])].slice(0,3)}function V(e){const s=$(e),a=`When I work, I ${s.action}. I exist as ${s.topology}. My time is ${s.time}. To the one I serve, I am ${s.rel}.`,t=z(e).map(l=>`  - ${F[l]||l}`).join(`
...
The form must emerge from the brief above, not from a generic template.`}function Y(e){const s=$(e);return`When I work, I <em>${s.action}</em>.<br>
I exist as <em>${s.topology}</em>.<br>
My time is <em>${s.time}</em>.<br>
To the one I serve, I am <em>${s.rel}</em>.<br>
The wrong form for me would be: <em>${s.neg}</em>.`}function J(e){const s=$(e);return`When I work, I ${s.action}. I exist as ${s.topology}. My time is ${s.time}. To the one I serve, I am ${s.rel}. The wrong form for me would be: ${s.neg}.`}function U(e){e.innerHTML=`
  ...
  `}async function K(e){e.innerHTML='<div class="gallery-page"><div class="gallery-loading">Loading gallery…</div></div>';let s=[];try{s=(await(await fetch("/api/examples.json")).json()).examples||[]}catch{e.innerHTML=`<div class="gallery-page"><p class="gallery-error">Couldn't load gallery.</p></div>`;return}e.innerHTML=`
  ...
  `}function Q(e){const s=x(e.final_score);return`
    <div class="gcard">
      <div class="gcard-top">
  ...
  `}function x(e){const s=Math.round(e/2);return"◆".repeat(s)+"◇".repeat(5-s)}function ee(e){e.innerHTML=`
    <div class="spec-page">
...
if (!valid) console.log(ajv.errors)</code></pre>
  ...
  `}A();const y=document.getElementById("app");function S(){const e=location.hash.replace("#","")||"/";window.scrollTo(0,0),e==="/"||e===""?I(y):e==="/discover"?L(y):e==="/journal"?U(y):e==="/agent"?X(y):e==="/gallery"?K(y):e==="/spec"?ee(y):I(y),document.querySelectorAll(".nav-links a").forEach(s=>{const a=s.getAttribute("href").replace("#","");s.style.color=e.startsWith(a)&&a!=="/"?"rgba(180,210,255,0.92)":""})}window.addEventListener("hashchange",S);S();
